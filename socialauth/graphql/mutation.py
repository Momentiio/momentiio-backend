...
class SocialLoginMutation(DjangoModelFormMutation):
    class Meta:
        form_class = LoginForm

    authUser = Field(UserType)
...
class SocialSignupMutation(DjangoModelFormMutation):
    ...
    def mutate_and_get_payload(cls, root, info, **input):
        if not cls.is_open(info.context):
            raise GraphQLError('there seems to have been an error')
        return
